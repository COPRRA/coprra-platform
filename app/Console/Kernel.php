...
class Kernel extends ConsoleKernel
{
    ...
    private function scheduleSystemMaintenance(Schedule $schedule, LoggerInterface $logger): void
    {
        ...
        $schedule->command('db:monitor')->hourly()
            ->onFailure(static function () use ($logger): void {
    ...
    private function scheduleCopRraTasks(Schedule $schedule, LoggerInterface $logger): void
    {
        ...
        $schedule->command('exchange-rates:update')->dailyAt('02:00')
            ->onSuccess(static function () use ($logger): void {
    ...
    private function schedulePriceJobs(Schedule $schedule, LoggerInterface $logger): void
    {
        ...
        // Product Offers: Update prices from external stores daily
        $schedule->command('prices:update --limit=100')
            ->dailyAt('05:00')
            ->onSuccess(static function () use ($logger): void {
                $logger->info('Product prices updated successfully');
            })
            ->onFailure(static function () use ($logger): void {
                $logger->error('Product prices update failed');
            })
        ;

        // Price Alerts: Check for price drops hourly
        $schedule->command('alerts:check-prices')
            ->hourly()
            ->onSuccess(static function () use ($logger): void {
                $logger->info('Price alerts check completed successfully');
            })
            ->onFailure(static function () use ($logger): void {
                $logger->error('Price alerts check failed');
            })
        ;
    }
