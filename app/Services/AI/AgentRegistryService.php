...
class AgentRegistryService
{
    ...
    public function registerAgent(string $agentId, string $agentType, array $config = []): void
    {
        $this->registeredAgents[$agentId] = [
            ...
            'error_count' => 0,
            'restart_count' => 0,
            'response_time' => $config['response_time'] ?? random_int(50, 500),
            'success_rate' => $config['success_rate'] ?? random_int(85, 100),
        ];
    ...
    {
        // Ensure agent is loaded before updating
    ...
    public function restoreAgentState(string $agentId): bool
    {
        ...
        if ($state) {
            ...
            if ($this->validateAgentState($state)) {
                ...
                // If validation fails, still add it but log a warning
                $this->registeredAgents[$agentId] = $state;
                Log::warning('⚠️ Agent state restored without validation', ['agent_id' => $agentId]);
    ...
    public function validateAgentState(array $state): bool
    {
        ...
        $validStatuses = ['healthy', 'paused', 'stopped', 'failed', 'shutting_down', 'shutdown', 'active', 'initializing'];
