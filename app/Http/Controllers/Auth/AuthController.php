...
class AuthController extends Controller
{
    ...
    public function sendResetLinkEmail(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            // Always show success message to user, even if email sending fails
            // The error will be logged but not shown to the user
            return back()->with([
                'status' => 'We have emailed your password reset link!'
            ]);
        } catch (\Exception $e) {
            // Log the error but still show success message to user
            \Illuminate\Support\Facades\Log::error('Password reset email failed', [
                'email' => $request->email,
                'error' => $e->getMessage(),
            ]);

            // Show success message even if email sending failed
