<?php

declare(strict_types=1);

namespace Tests\Unit\Models;

use App\Models\AuditLog;
use App\Models\User;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Illuminate\Database\Eloquent\Relations\MorphTo;
use PHPUnit\Framework\Attributes\CoversClass;
use Tests\TestCase;

/**
 * Unit tests for the AuditLog model.
 *
 * @internal
 */
#[CoversClass(AuditLog::class)]
final class AuditLogTest extends TestCase
{
    /**
     * Test fillable attributes.
     */
    public function testFillableAttributes(): void
    {
        $fillable = [
            'event',
            'auditable_type',
            'auditable_id',
            'user_id',
            'ip_address',
            'user_agent',
            'old_values',
            'new_values',
            'metadata',
            'url',
            'method',
        ];

        self::assertSame($fillable, (new AuditLog())->getFillable());
    }

    /**
     * Test casts.
     */
    public function testCasts(): void
    {
        $casts = [
            'id' => 'int',
            'old_values' => 'array',
            'new_values' => 'array',
            'metadata' => 'array',
        ];

        self::assertSame($casts, (new AuditLog())->getCasts());
    }

    /**
     * Test user relation is a BelongsTo instance.
     */
    public function testUserRelation(): void
    {
        $auditLog = new AuditLog();

        $relation = $auditLog->user();

        self::assertInstanceOf(BelongsTo::class, $relation);
        self::assertSame(User::class, $relation->getRelated()::class);
    }

    /**
     * Test auditable relation is a MorphTo instance.
     */
    public function testAuditableRelation(): void
    {
        $auditLog = new AuditLog();

        $relation = $auditLog->auditable();

        self::assertInstanceOf(MorphTo::class, $relation);
    }

    /**
     * Test scopeEvent applies correct where clause.
     */
    public function testScopeEvent(): void
    {
        $query = AuditLog::query()->event('created');

        self::assertSame('select * from "audit_logs" where "event" = ?', $query->toSql());
        self::assertSame(['created'], $query->getBindings());
    }

    /**
     * Test scopeForUser applies correct where clause.
     */
    public function testScopeForUser(): void
    {
        $query = AuditLog::query()->forUser(1);

        self::assertSame('select * from "audit_logs" where "user_id" = ?', $query->toSql());
        self::assertSame([1], $query->getBindings());
    }

    /**
     * Test scopeForModel applies correct where clause.
     */
    public function testScopeForModel(): void
    {
        $query = AuditLog::query()->forModel('App\Models\User');

        self::assertSame('select * from "audit_logs" where "auditable_type" = ?', $query->toSql());
        self::assertSame(['App\Models\User'], $query->getBindings());
    }

    /**
     * Test scopeDateRange applies correct where clause.
     */
    public function testScopeDateRange(): void
    {
        $startDate = '2023-01-01';
        $endDate = '2023-12-31';

        $query = AuditLog::query()->dateRange($startDate, $endDate);

        self::assertSame('select * from "audit_logs" where "created_at" between ? and ?', $query->toSql());
        self::assertSame([$startDate, $endDate], $query->getBindings());
    }

    /**
     * Test getFormattedEventAttribute formats event correctly.
     */
    public function testGetFormattedEventAttribute(): void
    {
        $auditLog = new AuditLog(['event' => 'user_created']);

        self::assertSame('User created', $auditLog->formatted_event);
    }

    /**
     * Test getChangesSummaryAttribute returns 'No changes recorded' when no values.
     */
    public function testGetChangesSummaryAttributeNoChanges(): void
    {
        $auditLog = new AuditLog();

        self::assertSame('No changes recorded', $auditLog->changes_summary);
    }

    /**
     * Test getChangesSummaryAttribute returns 'No changes recorded' when only old_values.
     */
    public function testGetChangesSummaryAttributeOnlyOldValues(): void
    {
        $auditLog = new AuditLog(['old_values' => ['name' => 'old']]);

        self::assertSame('No changes recorded', $auditLog->changes_summary);
    }

    /**
     * Test getChangesSummaryAttribute returns 'No changes recorded' when only new_values.
     */
    public function testGetChangesSummaryAttributeOnlyNewValues(): void
    {
        $auditLog = new AuditLog(['new_values' => ['name' => 'new']]);

        self::assertSame('No changes recorded', $auditLog->changes_summary);
    }

    /**
     * Test getChangesSummaryAttribute returns summary of changes.
     */
    public function testGetChangesSummaryAttributeWithChanges(): void
    {
        $auditLog = new AuditLog([
            'old_values' => ['name' => 'old', 'email' => 'old@example.com'],
            'new_values' => ['name' => 'new', 'email' => 'new@example.com'],
        ]);

        $summary = $auditLog->changes_summary;

        self::assertStringContainsString('name: old → new', $summary);
        self::assertStringContainsString('email: old@example.com → new@example.com', $summary);
    }

    /**
     * Test getChangesSummaryAttribute handles boolean values.
     */
    public function testGetChangesSummaryAttributeBooleanValues(): void
    {
        $auditLog = new AuditLog([
            'old_values' => ['active' => false],
            'new_values' => ['active' => true],
        ]);

        $summary = $auditLog->changes_summary;

        self::assertStringContainsString('active: 0 → 1', $summary);
    }

    /**
     * Test getChangesSummaryAttribute handles null values.
     */
    public function testGetChangesSummaryAttributeNullValues(): void
    {
        $auditLog = new AuditLog([
            'old_values' => ['description' => 'old'],
            'new_values' => ['description' => null],
        ]);

        $summary = $auditLog->changes_summary;

        self::assertStringContainsString('description: old → null', $summary);
    }

    /**
     * Test getChangesSummaryAttribute returns empty string when no actual changes.
     */
    public function testGetChangesSummaryAttributeNoActualChanges(): void
    {
        $auditLog = new AuditLog([
            'old_values' => ['name' => 'same'],
            'new_values' => ['name' => 'same'],
        ]);

        self::assertSame('', $auditLog->changes_summary);
    }
}
