...
const handleAddRemove = async button => {
  ...
  try {
    ...
    if (!isCurrentlyAdded) {
      notify('Product added to comparison successfully!', 'success', 'Added');
    } else {
      notify('Product removed from comparison.', 'info', 'Removed');
    }

    // Only reload if we're on the compare page
