...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-col md:flex-row gap-6">
            ...
            <div class="md:w-3/4">
                ...
                <p class="text-lg text-gray-600 dark:text-gray-400 mb-4">
                    {{ __('by') }} <span class="font-semibold">{{ $product->brand->name }}</span>
                </p>
                @endif
    ...
    @else
        <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6 text-center">
            ...
            <button id="refresh-prices-btn-empty"
                    class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md transition">
                <i class="fas fa-sync-alt mr-2"></i>
                {{ __('Try Again') }}
            </button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const refreshBtnEmpty = document.getElementById('refresh-prices-btn-empty');
    if (refreshBtnEmpty) {
        refreshBtnEmpty.addEventListener('click', function(e) {
            e.preventDefault();
            refreshPrices(this);
        });
    }
