                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            <form method="POST" action="{{ route('price-alerts.destroy', $alert->id) }}" class="inline delete-alert-form" data-alert-id="{{ $alert->id }}">
                                                @csrf
                                                @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.delete-alert-form').forEach(form => {
        form.addEventListener('submit', function(e) {
            ...
            if (confirm('{{ __('Are you sure you want to delete this price alert?') }}')) {
                ...
                .then(data => {
                    if (data.success || response.ok) {
                        ...
                        if (typeof notify === 'function') {
                            notify('{{ __('Price alert deleted successfully!') }}', 'success', '{{ __('Success') }}');
                        }
                    } else {
