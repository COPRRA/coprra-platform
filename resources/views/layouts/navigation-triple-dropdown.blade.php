...
<div class="flex items-center space-x-2">
    ...
    <div x-data="{ languageOpen: false }" class="relative">
        ...
        <div
            ...
            style="display: none;">
            <form method="POST" action="{{ route('locale.language') }}" id="language-form">
                @csrf
    ...
    <div x-data="{ countryOpen: false }" class="relative">
        <button
            ...
            @endphp
            <span class="hidden sm:inline" x-show="!countryOpen">{{ $currentCountry?->flag ?? '' }} {{ $currentCountry?->native_name ?? $currentCountry?->name ?? __('Select Country') }}</span>
        </button>
        <div
            x-show="countryOpen"
            x-transition:enter="transition ease-out duration-200"
            ...
            style="display: none;">
            <form method="POST" action="{{ route('locale.country') }}" id="country-form">
                @csrf
    ...
    <div x-data="{ currencyOpen: false }" class="relative">
        ...
        <div
            ...
            style="display: none;">
            <form method="POST" action="{{ route('locale.currency') }}" id="currency-form">
                @csrf
